...
library(skimr)
library(psych)
library(naniar) # use install.packages('naniar') in the console
```
...
```{r}
raw <- read.dta13("anes_timeseries_2016_dta/anes_timeseries_2016_Stata13.dta")
```
...
```{r}
# Replace all values for feeling thermometers less than 0 or greater than 100 with N/A
replaced_ft <- subset(clean, select=c(ft_obama:ft_blm)) %>%
  replace_with_na_all(condition = ~.x < 0 | .x > 100)

# Merge na-replaced dataframe with the cleaned dataframe
clean <- cbind(subset(clean, select=c(ideology:radio_no_radio)), replaced_ft)
```
